...
(
  ...
  output           stall_Mhl,
  output           stall_Whl,
  output [1:0] rs1_byp_sel_Dhl,
  output [1:0] rs2_byp_sel_Dhl,
  ...
  // Detects whether the instruction in D is a load. Pipelined to X so
  // we can use it for the load-use hazard stall check.
  ...
  // each signal represents whether a forwarded value is available and needed from a given stage
  // cnditions for a valid bypass:
  // 1. instr in D is valid and reads rs1/rs2
  // 2. instr in the later stage is valid
  // 3. later instr writes to a register (rf_wen)
  // 4. write address matches the read address
  // 5. write address is not x0
  // priority: X > M > W (X is most recent and takes precedence)
  ...
  // bypass mux select outputs
  // X wins over M wins over W wins over regfile.
  assign rs1_byp_sel_Dhl = rs1_X_byp_Dhl ? 2'd1 :
                           rs1_M_byp_Dhl ? 2'd2 :
                           rs1_W_byp_Dhl ? 2'd3 : 2'd0;

  assign rs2_byp_sel_Dhl = rs2_X_byp_Dhl ? 2'd1 :
                           rs2_M_byp_Dhl ? 2'd2 :
                           rs2_W_byp_Dhl ? 2'd3 : 2'd0;
